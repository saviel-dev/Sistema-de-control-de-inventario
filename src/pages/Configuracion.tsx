import { Settings, User, Store, Bell, Database, Palette, Save, Loader2, Download, Layout } from 'lucide-react';
...
const Configuracion = () => {
  ...
  const { businessInfo, notifications, navigationType, loading, saving, updateBusinessInfo, updateNotifications, updateNavigationType, saveSettings } = useSettings();
  const { user } = useAuth();
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 rounded-lg text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0 ${activeTab === tab.id
              ...
              <div className="bg-card rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  {[
                    ...
                    { id: 'movimientosInventario', label: 'Nuevos movimientos de insumos', description: 'Notificaciones en tiempo real de entradas y salidas' },
              ...
              <div className="bg-card rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 space-y-4">
                  <div className="p-4 border border-border rounded-lg">
                    <h3 className="font-medium text-foreground mb-2">Tipo de Navegación</h3>
                    <p className="text-sm text-muted-foreground mb-4">Selecciona el estilo de navegación de la interfaz.</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <button
                        onClick={() => updateNavigationType('sidebar')}
                        className={`p-4 rounded-lg border-2 transition-all ${navigationType === 'sidebar' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'}
                          `}
                      >
                        <div className="w-full h-24 bg-secondary rounded mb-3 flex gap-2 p-2">
                          <div className="w-1/4 bg-card rounded shadow flex flex-col gap-1 p-1">
                            <div className="h-2 bg-primary/50 rounded" />
                            <div className="h-1.5 bg-muted rounded" />
                            <div className="h-1.5 bg-muted rounded" />
                            <div className="h-1.5 bg-muted rounded" />
                          </div>
                          <div className="flex-1 bg-card/50 rounded" />
                        </div>
                        <p className="text-sm font-medium text-foreground mb-1">Barra Lateral (Sidebar)</p>
                        <p className="text-xs text-muted-foreground">Navegación en una barra lateral izquierda</p>
                      </button>
                      <button
                        onClick={() => updateNavigationType('navbar')}
                        className={`p-4 rounded-lg border-2 transition-all ${navigationType === 'navbar' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'}
                          `}
                      >
                        <div className="w-full h-24 bg-secondary rounded mb-3 flex flex-col gap-2 p-2">
                          <div className="h-1/4 bg-card rounded shadow flex items-center gap-1 px-2">
                            <div className="h-1.5 w-1.5 bg-primary/50 rounded-full" />
                            <div className="h-1 flex-1 bg-muted rounded" />
                          </div>
                          <div className="flex-1 bg-card/50 rounded" />
                        </div>
                        <p className="text-sm font-medium text-foreground mb-1">Barra Superior (Navbar)</p>
                        <p className="text-xs text-muted-foreground">Navegación en una barra superior horizontal</p>
                      </button>
                    </div>
                    {navigationType === 'navbar' && (
                      <div className="mt-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                        <p className="text-sm text-yellow-700 dark:text-yellow-400">
                          <strong>Nota:</strong> La navegación por barra superior estará disponible en una futura actualización. Por ahora, solo está disponible la navegación por barra lateral.
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4 border border-border rounded-lg">
                    ...
                    <div className="flex gap-4">
                      <button
                        ...
                        className={`flex-1 p-4 rounded-lg border-2 transition-colors ${theme === 'light' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'
                          }`}
                      >
                      ...
                      <button
                        ...
                        className={`flex-1 p-4 rounded-lg border-2 transition-colors ${theme === 'dark' ? 'border-primary bg-primary/10' : 'border-border hover:border-primary/50'
                          }`}
                      >
